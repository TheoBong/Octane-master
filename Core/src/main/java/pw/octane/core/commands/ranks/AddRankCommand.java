...
public class AddRankCommand extends MCommand {

    private CoreModule module;

    public AddRankCommand(Module module, String name) {
        super(module, name);
        this.module = (CoreModule) module;
        this.setPermission("core.commands.addrank");
    }
    ...
    public void execute(CommandSender sender, String[] args, String alias) {
        if(args.length > 1) {
            ...
            if(!profile.getRanks().contains(rank.getUuid())) {
                profile.getRanks().add(rank.getUuid());
